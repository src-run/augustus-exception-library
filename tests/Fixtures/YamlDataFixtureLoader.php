...
class YamlDataFixtureLoader
{
    ...
    public function load(string $context = null): \Generator
    {
        ...
        foreach ($cont as $c) {
            if (!array_key_exists($c, $data) || !array_key_exists('data', $data[$c])) {
                throw new \InvalidArgumentException(sprintf('Failed to retrieve fixture data context: "%s".', $c));
            }
    ...
    private function readData(): array
    {
        try {
            ...
            throw new \RuntimeException(sprintf('Failed to parse YAML from resolved file target "%s": %s.', $this->resolveFilePath(), $exception->getMessage()));
        }
    }
